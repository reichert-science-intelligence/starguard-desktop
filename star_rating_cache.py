...
def star_label(rating: float) -> tuple:
    """Return (stars_str, color_hex, label) for a given rating."""
...
class StarRatingCacheDB:
    """
    Manages Google Sheets read/write for StarGuard forecast cache.
    Credentials: GSHEETS_CREDS_JSON (HF Secret) or service_account.json
    Sheet name:  STAR_CACHE_SHEET_ID env var or
    ...
    def _connect(self):
        try:
            ...
            # Load last cached timestamp from sheet
            rows = self.sheet.get_all_values()
            self.cache_count = max(0, len(rows) - 1)
            if self.cache_count > 0:
                # timestamp is column index 1 (0-based)
...
def cache_forecast(db: StarRatingCacheDB, forecast: dict) -> dict:
    """
    Write a forecast run to the Google Sheets cache.

    forecast keys:
        contract_id, plan_name, measurement_year,
        current_star_rating, projected_star_rating,
        top_gap_measure, gaps_open, gaps_closed,
        hedis_completion_rate, hcc_risk_score,
        cahps_score, roi_projection, confidence_level,
        claude_narrative, cached_by
    """
    if not db.connected:
        return {"success": False, "error": f"Cloud disconnected: {db.last_error}"}

    try:
        # Mark all previous FRESH rows as STALE for this contract
...
def _mark_prior_stale(db: StarRatingCacheDB, contract_id: str):
    """Internal: mark all FRESH rows for this contract as STALE."""
...
def fetch_forecast_history(
    ...
    Returns columns: timestamp, projected_star_rating, current_star_rating
    """
    if not db.connected:
        return pd.DataFrame()
    try:
...
def fetch_cache_summary(db: StarRatingCacheDB) -> dict:
    """KPI summary for cache dashboard row."""
    if not db.connected:
        return {}
    try:
        records = db.sheet.get_all_records()
