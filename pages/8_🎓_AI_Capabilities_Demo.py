...
Showcase Context Engineering and Agentic RAG capabilities
"""
...
# Add project root to path for context engineering and agentic RAG
...
st.set_page_config(
    page_title="AI Capabilities Demo",
    page_icon="🎓",
    layout="wide"
)
...
# Custom filters for AI Capabilities Demo
def render_ai_demo_filters():
    st.markdown("<p style='color: white; font-size: 1rem; font-weight: 600;'>🎓 AI Demo Filters</p>", unsafe_allow_html=True)
    
    # AI capability focus - Radio buttons to show all options
    st.markdown("**AI Capability Focus**")
    ai_focus = st.radio(
        "Select AI Capability:",
        options=["All Capabilities", "Predictive Analytics", "Recommendations", "Natural Language", "Automated Insights"],
        index=0,
        key="ai_demo_focus",
        help="Focus on specific AI capability"
    )
    ...
    show_advanced = st.checkbox(
        "Show Advanced Features",
        value=True,
        key="ai_demo_advanced",
        help="Display advanced AI capabilities"
    )
    
    st.markdown("---")
    
    # Model selection - Radio buttons to show all options
    st.markdown("**AI Model**")
    model_type = st.radio(
        "Select AI Model:",
        options=["Local LLM", "On-Premises GPT", "Ensemble Model"],
        index=0,
        key="ai_demo_model",
        help="AI model for demonstrations"
    )

render_standard_sidebar(
    membership_slider_key="membership_slider_ai_capabilities",
    start_date_key="sidebar_start_date_ai_capabilities",
    end_date_key="sidebar_end_date_ai_capabilities",
    custom_filters=[render_ai_demo_filters]
)
...
""", unsafe_allow_html=True)
st.markdown("<p style='text-align: center; margin-top: 0; margin-bottom: 0.75rem; font-size: 1rem;'>Demonstration of AI-powered HEDIS optimization capabilities</p>", unsafe_allow_html=True)
...
# Enhanced AI Capabilities Visualizations
st.header("🤖 AI Capability Demonstrations")

# Get data for demonstrations
from utils.queries import get_roi_by_measure_query
from utils.enhanced_charts import create_wow_pie_chart, create_wow_heatmap, create_wow_bar_chart
import pandas as pd

demo_query = get_roi_by_measure_query(
    start_date.strftime("%Y-%m-%d"),
    end_date.strftime("%Y-%m-%d")
)
demo_df = execute_query(demo_query)

if not demo_df.empty:
    # Scale data
    demo_df_scaled = demo_df.copy()
    demo_df_scaled['total_investment'] = demo_df_scaled['total_investment'].astype(float) * scale_factor
    demo_df_scaled['revenue_impact'] = demo_df_scaled['revenue_impact'].astype(float) * scale_factor
    demo_df_scaled['success_rate'] = (demo_df_scaled['successful_closures'] / demo_df_scaled['total_interventions'].replace(0, 1) * 100).round(1)
    
    # Ensure no zero data
    from utils.data_validation import ensure_no_zero_data
    demo_df_scaled = ensure_no_zero_data(demo_df_scaled, columns=['total_investment', 'revenue_impact', 'successful_closures', 'total_interventions', 'roi_ratio', 'success_rate'])
    demo_df_scaled['success_rate'] = (demo_df_scaled['successful_closures'] / demo_df_scaled['total_interventions'] * 100).round(1)
    ...
    selected_tab = st.radio(
        "Select View",
        ["📊 Predictive Analytics", "💡 Recommendations", "🔍 NLP Query", "📈 Automated Insights"],
        horizontal=True,
    ...
    if selected_tab == "📊 Predictive Analytics":
        st.subheader("📊 Predictive Analytics Demo")
        st.markdown("**Forecast gap closure rates based on historical patterns**")
        
        # Top measures by success rate
        top_success = demo_df_scaled.nlargest(8, 'success_rate')
        fig_pie = create_wow_pie_chart(
            df=top_success,
            values_col="successful_closures",
            names_col="measure_code",
            title="Top 8 Measures: Successful Closures Distribution",
            color_palette="vibrant",
            hole=0.4
        )
        st.plotly_chart(fig_pie, use_container_width=True, config={'responsive': True, 'displayModeBar': False}, key="ai_demo_pie")
        
        st.info("💡 **AI Prediction:** Based on current trends, these measures show highest closure probability.")
    
    elif selected_tab == "💡 Recommendations":
        st.subheader("💡 Recommendation Engine Demo")
        st.markdown("**AI-recommended optimal intervention strategies**")
        
        # Calculate efficiency score
        demo_df_scaled['efficiency_score'] = (demo_df_scaled['roi_ratio'] * demo_df_scaled['success_rate'] / 100).round(2)
        top_recommendations = demo_df_scaled.nlargest(10, 'efficiency_score')
        
        fig_rec = create_wow_bar_chart(
            df=top_recommendations,
            x_col="measure_code",
            y_col="efficiency_score",
            color_col="roi_ratio",
            title="AI Recommendations: Top 10 Measures by Efficiency Score",
            x_label="Measure Code",
            y_label="Efficiency Score (ROI × Success Rate)",
            color_palette="gradient_green",
            show_values=True
        )
        st.plotly_chart(fig_rec, use_container_width=True, config={'responsive': True, 'displayModeBar': False}, key="ai_demo_recommendations")
        
        st.success(f"🎯 **Top Recommendation:** {top_recommendations.iloc[0]['measure_code']} - Highest efficiency score of {top_recommendations.iloc[0]['efficiency_score']:.2f}")
    
    elif selected_tab == "🔍 NLP Query":
        st.subheader("🔍 Natural Language Query Demo")
        st.markdown("**Ask questions about your data**")
        
        # Create heatmap of investment vs success rate
        demo_df_scaled['investment_tier'] = pd.cut(demo_df_scaled['total_investment'], bins=5, labels=['Very Low', 'Low', 'Medium', 'High', 'Very High'])
        demo_df_scaled['success_tier'] = pd.cut(demo_df_scaled['success_rate'], bins=5, labels=['Very Low', 'Low', 'Medium', 'High', 'Very High'])
        
        heatmap_data = demo_df_scaled.groupby(['investment_tier', 'success_tier']).agg({
            'roi_ratio': 'mean',
            'measure_code': 'count'
        }).reset_index()
        heatmap_data.columns = ['Investment Tier', 'Success Tier', 'Avg ROI', 'Measure Count']
        
        if len(heatmap_data) > 0:
            fig_heatmap = create_wow_heatmap(
                df=heatmap_data,
                x_col="Investment Tier",
                y_col="Success Tier",
                values_col="Avg ROI",
                title="Investment vs Success Rate: ROI Heatmap",
                color_palette="sunset"
            )
            st.plotly_chart(fig_heatmap, use_container_width=True, config={'responsive': True, 'displayModeBar': False}, key="ai_demo_heatmap")
        
        st.info("💡 **Query Example:** 'Which measures have high investment but low success rates?' - Answer shown in heatmap above.")
    
    elif selected_tab == "📈 Automated Insights":
        st.subheader("📈 Automated Insights Demo")
        st.markdown("**AI-generated performance insights**")
        
        # Key insights
        insights = []
        if demo_df_scaled['roi_ratio'].max() > 2.0:
            best_measure = demo_df_scaled.loc[demo_df_scaled['roi_ratio'].idxmax()]
            insights.append(f"🌟 **Highest ROI:** {best_measure['measure_code']} with {best_measure['roi_ratio']:.2f}x ROI")
        
        if demo_df_scaled['success_rate'].max() > 50:
            best_success = demo_df_scaled.loc[demo_df_scaled['success_rate'].idxmax()]
            insights.append(f"✅ **Best Success Rate:** {best_success['measure_code']} with {best_success['success_rate']:.1f}%")
        
        avg_roi = demo_df_scaled['roi_ratio'].mean()
        insights.append(f"📊 **Portfolio Average ROI:** {avg_roi:.2f}x")
        
        total_net = (demo_df_scaled['revenue_impact'] - demo_df_scaled['total_investment']).sum()
        insights.append(f"💰 **Total Net Benefit:** ${total_net:,.0f}")
        
        for insight in insights:
            st.markdown(f"- {insight}")
        
        # Performance distribution
        fig_dist = create_wow_bar_chart(
            df=demo_df_scaled.sort_values('roi_ratio', ascending=False).head(12),
            x_col="measure_code",
            y_col="roi_ratio",
            color_col="success_rate",
            title="Automated Insight: ROI Distribution Across Measures",
            x_label="Measure Code",
            y_label="ROI Ratio",
            color_palette="ocean",
            show_values=True
        )
        st.plotly_chart(fig_dist, use_container_width=True, config={'responsive': True, 'displayModeBar': False}, key="ai_demo_insights")
else:
    st.info("📊 No data available for AI demonstrations. Please adjust filters or check data availability.")
