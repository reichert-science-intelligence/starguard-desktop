...
def star_cache_css() -> ui.tags.style:
    return ui.tags.style("""
        ...
        /* ── Confidence badge ── */
        .conf-high   { background:#1a3a2a; color:#10b981;
                       padding:2px 10px; border-radius:12px;
                       font-size:11px; font-weight:700; }
        .conf-medium { background:#1e3a5f; color:#60a5fa;
                       padding:2px 10px; border-radius:12px;
                       font-size:11px; font-weight:700; }
        .conf-low    { background:#2a1a1a; color:#f87171;
                       padding:2px 10px; border-radius:12px;
                       font-size:11px; font-weight:700; }
...
def star_rating_cache_panel() -> ui.div:
    """
    Full Star Rating Forecast Cache panel.
    Drop into StarGuard as: ui.nav_panel("⭐ Star Forecast Cache", star_rating_cache_panel())
    """
    return ui.div(
        star_cache_css(),
        ...
        ui.div(
            ...
            ui.input_action_button(
                "btn_refresh_cache", "↻ Refresh",
                class_="btn btn-sm btn-outline-secondary"
            ),
            class_="cache-sync-row"
        ),

        # ── Cache freshness banner (latest forecast) ──
        ...
        # ── Run & cache new forecast ──
        ui.card(
            ui.card_header("Run & Cache New Forecast"),
            ui.layout_columns(
                ui.input_text("fcst_contract_id", "Contract ID",
                              placeholder="e.g. H1234"),
                ui.input_text("fcst_plan_name", "Plan Name",
                              placeholder="e.g. BlueCare Medicare Advantage"),
                col_widths=[4, 8]
            ),
            ui.layout_columns(
                ui.input_numeric("fcst_year", "Measurement Year",
                                 value=2026, min=2020, max=2030),
                ui.input_numeric("fcst_current", "Current Stars",
                                 value=3.5, min=1.0, max=5.0, step=0.5),
                ui.input_numeric("fcst_projected", "Projected Stars",
                                 value=4.0, min=1.0, max=5.0, step=0.5),
                col_widths=[4, 4, 4]
            ),
            ui.layout_columns(
                ui.input_text("fcst_top_gap", "Top Gap Measure",
                              placeholder="e.g. CBP"),
                ui.input_numeric("fcst_gaps_open", "Gaps Open",
                                 value=0, min=0),
                ui.input_numeric("fcst_gaps_closed", "Gaps Closed",
                                 value=0, min=0),
                col_widths=[4, 4, 4]
            ),
            ui.layout_columns(
                ui.input_numeric("fcst_hedis_rate", "HEDIS Completion Rate",
                                 value=0.75, min=0.0, max=1.0, step=0.01),
                ui.input_numeric("fcst_hcc", "HCC Risk Score",
                                 value=1.0, min=0.0, max=5.0, step=0.01),
                ui.input_numeric("fcst_cahps", "CAHPS Score",
                                 value=80.0, min=0.0, max=100.0, step=0.1),
                col_widths=[4, 4, 4]
            ),
            ui.layout_columns(
                ui.input_numeric("fcst_roi", "ROI Projection ($)",
                                 value=0, min=0, step=1000),
                ui.input_select("fcst_confidence", "Confidence Level",
        ...
        ui.card(
            ...
            ui.layout_columns(
                ui.input_text("fcst_filter_contract", "Filter by Contract ID",
                              placeholder="Leave blank for all"),
                ui.input_action_button("btn_load_history", "Load History",
