...
def create_wow_scatter(
    ...
    if size_col and size_col in df.columns:
        ...
        if size_values.max() > size_values.min():
            normalized_size = (size_values - size_values.min()) / (
...
def create_wow_bar_chart(
    ...
    if color_col and color_col in df.columns:
        fig = px.bar(
            ...
            title=title,
            labels={
                x_col: x_label or format_column_label(x_col),
                y_col: y_label or format_column_label(y_col),
            },
        )
    else:
        fig = px.bar(
...
def create_wow_radar_chart(
    ...
    fig.update_layout(
        ...
        title=dict(
            text=title,
            font=dict(size=20, color=MEDICAL_THEME["primary"], family="Source Sans 3, sans-serif"),
            x=0.5,
            xanchor="center",
        ),
        showlegend=show_legend,
        legend=dict(orientation="v", yanchor="top", y=1, xanchor="left", x=1.1, font=dict(size=12))
        if show_legend
        else None,
        paper_bgcolor="white",
        plot_bgcolor="white",
