...
def format_date_display(date: datetime | str) -> str:
    """
...
def get_data_date_range() -> tuple[datetime, datetime] | None:
    """
    ...
    try:
        df = query(
            "SELECT MIN(intervention_date) as min_date, MAX(intervention_date) as max_date FROM member_interventions"
        )
...
def show_data_availability_warning(
    selected_start: datetime, selected_end: datetime
) -> dict[str, str] | None:
    """
    ...
    if date_range:
        ...
        available_start_db = (
            ...
            if hasattr(available_start, "strftime")
            else str(available_start)
        )
        available_end_db = (
            available_end.strftime("%Y-%m-%d")
            if hasattr(available_end, "strftime")
            else str(available_end)
        )
