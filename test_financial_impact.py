...
for m in rating_analysis["measure_breakdown"]:
    ...
    print(
        f"  {weight_indicator} {m['measure_id']}: {m['rate']:.1%} -> {m['stars']:.1f} stars (weight: {m['weight']})"
    )
...
print("\n--- CMS Bonus Payments ---")
print(
    f"Current Bonus: {financial['current_bonus_pct']:.1%} = ${financial['current_bonus_payment']:,.0f}"
)
print(
    f"Projected Bonus: {financial['projected_bonus_pct']:.1%} = ${financial['projected_bonus_payment']:,.0f}"
)
...
opportunities = generate_gap_closure_recommendations(rating_analysis["measure_breakdown"], top_n=5)
